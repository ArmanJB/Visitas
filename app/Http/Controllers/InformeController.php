...
class InformeController extends Controller
{
    ...
    public function resumenTaller($desde, $hasta){
        $oficiales = DB::select("SELECT talleres.id_actividad, taller_oficial.id_oficial, CONCAT(oficiales.nombres, ' ', oficiales.apellidos) AS nombre
FROM taller_oficial
INNER JOIN oficiales ON taller_oficial.id_oficial = oficiales.id
LEFT JOIN talleres ON taller_oficial.id_taller = talleres.id
GROUP BY taller_oficial.id_oficial");
        $oficialesCant = DB::select("SELECT talleres.id_actividad, taller_oficial.id_oficial, CONCAT(oficiales.nombres, ' ', oficiales.apellidos) AS nombre
FROM taller_oficial
INNER JOIN oficiales ON taller_oficial.id_oficial = oficiales.id
LEFT JOIN talleres ON taller_oficial.id_taller = talleres.id");
        ...
        foreach ($oficiales as $key => $value) {
            ...
            $aux = [];
            $actOfc = DB::select("SELECT talleres.id_actividad, actividades.nombre, taller_oficial.id_oficial
FROM taller_oficial
LEFT JOIN talleres ON taller_oficial.id_taller = talleres.id
INNER JOIN actividades ON talleres.id_actividad = actividades.id
WHERE taller_oficial.id_oficial = $value->id_oficial
GROUP BY talleres.id_actividad");
            $actOfcCant = DB::select("SELECT talleres.id_actividad, actividades.nombre, taller_oficial.id_oficial
FROM taller_oficial
LEFT JOIN talleres ON taller_oficial.id_taller = talleres.id
INNER JOIN actividades ON talleres.id_actividad = actividades.id
WHERE taller_oficial.id_oficial = $value->id_oficial");
